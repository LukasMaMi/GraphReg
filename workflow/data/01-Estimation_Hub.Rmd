...
for (rep in 1:num_repetitions) {
  for (i in seq_along(configs)) {
    ...
    # Lambda path plots
    plot_lambda_path <- function(config, rep, prefix, dir_path) {
      ...
      plot(out.p, legends = TRUE, show = c("stars", "gcd", "gcd_prior"))
      dev.off()
    }

    plot_lambda_path(config, rep, "Plot_gcd", lambda_dir)
    plot_lambda_path(config, rep, "Plot_gcd_prior", lambda_dir)
    ...
    save(num_repetitions, configs, dir_path, dir_path2, dir_path_results, out.p, file = hub_settings_file)
