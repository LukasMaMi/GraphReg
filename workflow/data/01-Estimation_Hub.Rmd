...
plot_and_save_gcm <- function(output, criterion, config, rep, dir_path) {
  if ("gcm" %in% names(output[[criterion]])) {
    ...
    # Check if GCM is a matrix and not empty
    if(is.matrix(gcm) && length(gcm) > 0) {
      # Further check if values are within the range [-1, 1]
      if(min(gcm) >= -1 && max(gcm) <= 1) {
        ...
        # Handle the case where GCM values are not in the expected range
        cat(sprintf("Skipping plot for %s: GCM values are not in [-1, 1]\n", criterion))
      }
    } else {
      # Handle the case where GCM is not a proper matrix or is empty
      cat(sprintf("Skipping plot for %s: GCM is empty\n", criterion))
    }
  }
}
...
for (rep in 1:num_repetitions) {
  for (i in seq_along(configs)) {
    ...
    categorized_info$additional_metrics$lb.index <- out.p[["stars"]][["lb.index"]]
    categorized_info$additional_metrics$ub.index <- out.p[["stars"]][["ub.index"]]
    
    # Bounding down again Stars
    if (!is.null(categorized_info$additional_metrics$lb.index)) {
      stars_bound <- categorized_info[["additional_metrics"]][["ind_bound"]]
      categorized_info$raw_summary$stars <- categorized_info$raw_summary$stars[stars_bound]
    }
    ...
    plot_lambda_path(config, rep, "Plot_gcd", lambda_dir, show = c("stars", "gcd_spearman", "gcd_kendall", "gcd_latentcor",
                                                                   "gcd_pseudo_spearman", "gcd_pseudo_kendall", "gcd_pseudo_latentcor"))
    ...
    for (meth in c("spearman", "kendall", "latentcor")) {
      gcm_true <- my.gcvec(true_graph, orbind = c(0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11), method = meth, return_gcm = TRUE)
      gcm_true_pseudo <- my.gcvec(true_graph, orbind = c(0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11), method = meth,
