...
for(rep in 1:num_repetitions) {
  ...
  for(cfg in configs) {
    ...
    # Allowed range for off-diagonal elements
    rho <- list(lower = -1, upper = 1)       # 1) 
    #rho <- list(lower = -1, upper = -0.05)  # 2) 
    #rho <- list(lower = 0.05, upper = 1)    # 3) 
    #rho <- list(lower = -1, upper = -0.01)  # 4) 
    ...
    # Lambda path
    lambda_path  <- getLamPath(max = getMaxCov(ER_data, cov = F, abs = T, diag = T), min = 0.001, len = 40)
    ...
    save(ER, ER_data, true_graph, lambda, sparsity, act_sparsity, true_prec, lambda_path, b, N, file = file_name) 
...
# Configurations and number of repetitions as previously defined
configs <- list(
  list(n=800, p=40),
  list(n=400, p=100),
  list(n=200, p=200),
...
# Loop through each configuration and repetition
for(cfg in configs) {
  for(rep in 1:num_repetitions) {
    ...
    print(paste("Data for configuration n =", n, "p =", p, "Repetition =", rep))
    print("Desired Sparsity Level:")
    print(sparsity)
    print("Actual Sparsity Level:")
    print(act_sparsity)
    #print("True Graph:")
    #print(true_graph)
    print("Omega:")
    print(kappa(true_prec))
    #print(true_prec)
