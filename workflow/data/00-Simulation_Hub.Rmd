...
run_hub <- function(n, p, s = s, J = J, rho = rho, u = u, b = b, N = N) {
...
for (config in configs) {
  cat("Running experiment for configuration:", paste0("n=", config$n, " p=", config$p), "\n")
...
ggplot(df, aes(x=np_setting)) + 
  ...
  geom_line(aes(y=F1_gcd, color="GCD", group="GCD"), linetype="dotted", color="brown", linewidth=1.5) + 
  ...
  labs(title = "F1-scores Across Different Configurations", y = "F1-score") + 
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14)) + 
  scale_color_manual(values=c("Stars"="lightblue", "GCD"="brown", "Oracle"="black"), 
                     name="Method",
