...
for(rep in 1:num_repetitions) {
  ...
  for(cfg in configs) {
    ...
    save(hub, hub_data, true_graph, true_cov, file = file_name)
  }
}
...
hub_data <- hub$data
true_graph <- as.matrix(hub$theta)
true_graph <- true_graph * lower.tri(true_graph, diag = TRUE)
true_cov <- hub$sigma
...
# Compute metrics for Oracle graph
...
# Compute metrics for Stars graph
...
# Compute metrics for GStars graph
...
for(rep in 1:num_repetitions){
  ...
  #Lambda path
  lambda_path  <- getLamPath(max = getMaxCov(hub_data), min = 0.001, len = 30)
  lambda <- list(lambda=lambda_path)
...
diag(oracle_graph) <- 0
  ...
  return(list(F1_stars = F1_scores[["stars"]], F1_gcd = F1_scores[["gstars"]], 
