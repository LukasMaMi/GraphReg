...
my.batch.pulsar <- function(data, funct = "QUIC", fargs=list(),
                    ...
                    use_pseudo_count = FALSE, pseudo_count_prob = 0.5, pseudo_count_strength = 1, five_node = FALSE) {
    ...
    if (!funct %in% c("QUIC", "glasso")) stop("Please choose either 'QUIC' or 'glasso' as function") # !("stars" %in% criterion)
        ...
        if ('gcd' %in% criterion || 'gcd_prior' %in% criterion) {
            ...
            for (meth in method) {
              ...
              if (use_pseudo_count == TRUE) {
                ...
                # Define aggfun for the current method + Pseudo Count
                aggfun <- function(res, ...) {
                    lapply(res, function(x) my.gcvec(x, method = meth, orbind = orbind, five_node = five_node,
                                                     pseudo_count = TRUE, pseudo_count_prob = pseudo_count_prob,
                                                     pseudo_count_strength = pseudo_count_strength, ...))
                }
                ...
                #est$test$lb.gcdpremerge_pseudo[[meth]] <- lb.gcdpremerge_pseudo[[meth]] ###
              }
                ...
                #est$test$lb.gcdpremerge[[meth]] <- lb.gcdpremerge[[meth]] ###
            }
        }
        ...
        if ('gcd' %in% criterion || 'gcd_prior' %in% criterion) { 
            ...
            for (meth in method) {
              ...
              if (use_pseudo_count == TRUE) {
                ...
                # Check if lb.gcdpremerge Pseudo Count for the current method exists
                if (!is.null(lb.gcdpremerge_pseudo[[meth]])) {
                ...
                if (!is.null(lb.gcdpremerge[[meth]])) {
                    # Create reduceGCDargs for the current method
              ...
              if (use_pseudo_count == TRUE) {
                ...
                makeGcdAggFun <- function(meth) {
                    function(res, ...) {
                        lapply(res, function(x) my.gcvec(x, method = meth, orbind = orbind, five_node = five_node,
                                                         pseudo_count = TRUE, 
                                                         pseudo_count_prob = pseudo_count_prob,
                                                         pseudo_count_strength = pseudo_count_strength, ...))
                    }
                }
                ...
                if (!is.null(reduceGCDargs_pseudo[[meth]])) {
                    # Processing and gathering results
...
my.gcvec <- function(graph, method, orbind, five_node, 
                     pseudo_count = FALSE, pseudo_count_prob, pseudo_count_strength) {
  ...
  if (pseudo_count == TRUE) {
    ...
    gdm <- modify_orb_count(orb_count = gdm, pseudo_count_prob = pseudo_count_prob, 
                            pseudo_count_strength = pseudo_count_strength)
  }
