...
my.batch.pulsar <- function(data, fun = c("QUIC"), fargs=list(),
    ...
    if (lb.stars) {
      ...
      if (refit == TRUE) {
        ...
        for (crit in criterion) {
          if (startsWith(crit, "gcd")) { 
            for (i in 1:length(fargs$lambda)) {
              gcdind <- which(est[[crit]]$summary == est[[crit]]$summary[[i]])
              gcdind <- gcdind + est$additional$ub.index - 1
              est[[crit]]$all_adj[[i]] <- fullmodel$path[[gcdind]]
            }
          }
        }
        ...
        if (!is.null(prior_graph)) {
          ...
          est$additional$hamming_dist <- numeric(length = length(fargs$lambda))
          est$additional$f1_score <- numeric(length = length(fargs$lambda))
    
          for (i in 1:length(fargs$lambda)) {
            gcdind <- which(fargs$lambda == fargs$lambda[[i]])
            gcdind <- gcdind + est$additional$ub.index - 1
            est$additional$hamming_dist[[i]] <- my.hamming(estimated = fullmodel$path[[gcdind]], actual = prior_graph) 
            est$additional$f1_score[[i]] <- my.f1_score(predicted = fullmodel$path[[gcdind]], actual = prior_graph)
          }
