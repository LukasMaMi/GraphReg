...
# Maybe check if we do not need to calculate gcm separate from gcv
# Option to make a non-monoton function monoton?
# Other dependency measures than correlation? Maybe Mutual information?
...
my.batch.pulsar <- function(data, fun = c("QUIC"), fargs=list(),
    ...
    # Integrated QUIC and GLASSO Methods
    if (fun == "QUIC") {
      # Using QUIC
      fun <- fun_QC
    } else if (fun == "GLASSO") {
      ...
      fun <- fun
      } else stop("Please choose either 'QUIC' or 'GLASSO' as function argument or define a function yourself by defining 'fun <-'")
        ...
        if (crit == "gcd" || crit == "gcd_prior") {
            ...
            for (meth in method) {
              ...
              gcdpremerge_gcm <- c(reduceGCDargs_gcm[[meth]]$init,
                               batchtools::reduceResultsList(reg = reg, ids = pulsar.jobs, fun = aggfun))
              #est[[paste("gcdpremerge_gcm", meth, sep = "_")]] <- gcdpremerge_gcm
    ...
    if (lb.stars) {
      ...
      pind <- ub.index:lb.est$opt.index # bounded indices 
      pinv <- setdiff(1:length(lb.est$summary), pind) # full indices
      est$additional$ind_bound <- pind
      est$additional$ind_full <- pinv
    ...
    # Determine optimal gcm according to optimal index
    for (crit in criterion) {
    ...
    if (lb.stars) {
      ...
      #######
      est$fargs <- fargs$lambda
      est$path <- fullmodel$path
      ########
      ...
      if (refit == TRUE) {
        ...
        for (crit in criterion) {
            ...
            if (startsWith(crit, "gcd")) {
                for (i in seq_along(fargs$lambda)) {
                    ...
                    gcdinds <- gcdinds + est$additional$ub.index - 1
        ...
        if (!is.null(prior_graph)) {
            ...
            est$oracle_f1$opt.index <- which.max(est$additional$f1_score)
            ...
            est$oracle_hamming$opt.index <- which.min(est$additional$hamming_dist)
...
calculateMeanMatrices <- function(data, nlams, rep.num) {
    ...
    for (i in 1:nlams) {
        matrices <- lapply(data, function(sublist) sublist[[i]])
        valid_matrices <- Filter(function(m) is.matrix(m) && nrow(m) > 0 && ncol(m) > 0, matrices)

        if (length(valid_matrices) > 0) {
            sum_matrix <- Reduce("+", valid_matrices)
            mean_matrix <- sum_matrix / length(valid_matrices)
            mean_matrices[[i]] <- mean_matrix
        } else {
            mean_matrices[[i]] <- NULL  # or some other placeholder indicating no valid data
        }
    }
...
stars.stability <- function(premerge, stars.thresh, rep.num, p, merge=NULL) { # Only 5 arguments!!!
    ...
    est$summary   <- cummax(est$summary) # !!!!!!!!!!!!!!!! Check, if applicable on gcd? Why not taking same approach as in stars?
...
```{r}
## Set up for custom function
lambda <- out.p$additional$lambda_bound
fullmodel <- out.p$est$path
prior_graph <- true_graph

        for (i in 1:length(lambda)) {
            gcdind <- which(lambda == lambda[[i]])
            gcdind <- gcdind + out.p$additional$ub.index - 1
            additional$hamming_dist[[i]] <- my.hamming(estimated = fullmodel[[gcdind]], actual = prior_graph) 
            additional$f1_score[[i]] <- my.f1_score(predicted = fullmodel[[gcdind]], actual = prior_graph)
          }

my.hamming(estimated = fullmodel[[2]], actual = prior_graph) 
```
