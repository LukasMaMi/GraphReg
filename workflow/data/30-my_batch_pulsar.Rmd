...
my.batch.pulsar <- function(data, fun=huge::huge, fargs=list(),
    ...
    if (lb.stars) {
        ...
        if ('gcd' %in% criterion || 'gcd_prior' %in% criterion) {
            # Initialize a list to store gcdpremerge results for each method
            lb.gcdpremerge_list <- list()
        
            for (meth in method) {
                # Define aggfun for the current method
                aggfun <- function(res, ...) {
                    lapply(res, function(x) my.gcvec(x, method = meth, orbind = orbind, ...))
                }
        
                # Processing and gathering results
                lb.gcdpremerge_list[[meth]] <- do.call(batchtools::reduceResultsList,
        ...
        if ('gcd' %in% criterion || 'gcd_prior' %in% criterion) { 
            # Initialize a list to store reduceGCDargs for each method
            reduceGCDargs_list <- list()
        
            for (meth in method) {
                # Check if lb.gcdpremerge for the current method exists
                if (!is.null(lb.gcdpremerge_list[[meth]])) {
                    # Create reduceGCDargs for the current method
                    reduceGCDargs_list[[meth]] <- list(init = lapply(lb.gcdpremerge_list[[meth]], 
    ...
    for (i in 1:length(criterion)) {
        ...
        if (crit == "gcd" || crit == "gcd_prior") {
            for (meth in method) {
                ...
                # Function factory to create gcdaggfun with bound method
                makeGcdAggFun <- function(meth) {
                    function(res, ...) {
                        lapply(res, function(x) my.gcvec(x, method = meth, orbind = orbind, ...))
                    }
                }
        
                # Create gcdaggfun for the current method
                gcdaggfun <- makeGcdAggFun(meth)
                
                # Check if reduceGCDargs for the current method exists
                if (!is.null(reduceGCDargs_list[[meth]])) {
                    # Processing and gathering results
                    gcdpremerge <- c(reduceGCDargs_list[[meth]]$init,
    ...
    for (crit in criterion) {
        if (crit == "gcd" || crit == "gcd_prior") {
            for (meth in method) {
                full_crit_name <- paste(crit, meth, sep = "_")
                if (full_crit_name %in% names(est)) {
                    gcdind <- which.min(est[[full_crit_name]]$summary)
                    gcdind <- gcdind + est$stars$ub.index - 1
                    est[[full_crit_name]]$opt.index <- gcdind
                }
            }
        }
    }
    ...
    # Bounded lambda path
    lambda_bound <- fargs$lambda
    est$lambda_bound <- lambda_bound
    ...
    for (crit in criterion) {
        ...
        if (crit == "gcd" || crit == "gcd_prior") {
              for (meth in method) {
                  full_crit_name <- paste(crit, meth, sep = "_")
                  if (full_crit_name %in% names(est)) {
                      opt_ind <- est[[full_crit_name]]$opt.index
                      est[[full_crit_name]]$opt.lambda <- signif(lambda_full[opt_ind], 3)
                  }
              }
          }
    }
...
.print.pulsar <- function(x, fin) {
    ...
    critext2 <- lapply(fin$criterion, function(cr) {
        if (grepl("gcd", cr)) {
            # Handle different gcd methods
            method_results <- lapply(fin$method, function(meth) {
                method_name <- paste(cr, meth, sep = "_")
                opt.ind <- x[[method_name]]$opt.ind
                optext  <- ifelse(is.null(opt.ind), "",
                  paste("... opt: index ", opt.ind, ", lambda ",
                  signif(fin$fargs$lambda[opt.ind], 3), sep=""))
                paste("  ", method_name, optext, sep="")
            })
            return(paste(method_results, collapse="\n"))
        } else {
            # Original code for other criteria
            opt.ind <- x[[cr]]$opt.ind
            optext  <- ifelse(is.null(opt.ind), "",
              paste("... opt: index ", opt.ind, ", lambda ",
              signif(fin$fargs$lambda[opt.ind], 3), sep=""))
...
my.gcvec <- function(graph, method, orbind, 
  ...
  gcm <- suppressMessages(latentcor::latentcor(gdm, method = "approx"))
  gcm <- gcm$K
  }
...
my.gcd.stability <- function(premerge, prior_graph, thresh, rep.num, p, nlams, use_prior = FALSE, method, orbind) { 
    est <- list()
    ...
    criterion_name <- if (use_prior) {
        paste("gcd_prior", method, sep = "_")
    } else {
        paste("gcd", method, sep = "_")
    }
...
############################
# my.get.opt.index <- function(obj, criterion = "gcd_spearman", ...) {
#     optind <- my.opt.index(obj, criterion)
#     if (!is.null(optind)) optind
#     if (criterion %in% c("gcd_spearman", "gcd_kendall", "gcd_latentcor", 
#                          "gcd_prior_spearman", "gcd_prior_kendall", "gcd_prior_latentcor")) {
#         if (is.null(obj$stars$lb.index) || !obj$stars$lb.index)
#             stop('Lower bound needed for gcd metrics (run with lb.stars=TRUE)')
...
# my.opt.index <- function(obj, criterion = 'gcd_spearman') {
#     .pcheck(obj)
...
my.get.opt.index <- function(obj, criterion = "stars") {
    optind <- my.opt.index(obj, criterion)
    if (!is.null(optind)) return(optind)
    
    if (startsWith(criterion, "gcd") || startsWith(criterion, "gcd_prior")) {
        if (is.null(obj$stars$lb.index) || !obj$stars$lb.index)
            stop('Lower bound needed for gcd metrics (run with lb.stars=TRUE)')
        gcdind <- which.min(getElement(obj, criterion)$summary)
        gcdind <- gcdind + obj$stars$ub.index - 1
...
my.opt.index <- function(obj, criterion = 'stars') {
    #.pcheck(obj)
    .critcheck(obj, criterion)
    getElement(obj, criterion)$opt.index
}
...
"my.opt.index<-" <- function(obj, criterion=names(value), value) {
    ...
    if (is.null(criterion)) criterion <- 'gcd_spearman'
    if (!is.null(value)) {
