...
my.batch.pulsar <- function(data, funct = c("QUIC"), fargs=list(),
    ...
    if (!is.null(prior_graph)) {
      ...
      for (i in 1:nlams) {
        ...
        gcdind <- gcdind + est$additional$ub.index - 1
        est$additional$hamming_dist[[i]] <- sum(tril(fullmodel$path[[gcdind]]) != tril(prior_graph))
...
my.hamming <- function(premerge, prior_graph) {
...
#         valid_matrices <- Filter(function(m) is.matrix(m) && nrow(m) > 0 && ncol(m) > 0, matrices)
# 
#         # Identify and print invalid matrices
#         invalid_matrices <- setdiff(matrices, valid_matrices)
#         if (length(invalid_matrices) > 0) {
#             cat(sprintf("Invalid matrices for lambda index %d:\n", i))
#             lapply(invalid_matrices, print)
#         }
#         
#         if (length(valid_matrices) > 0) {
#             sum_matrix <- Reduce("+", valid_matrices)
...
#             mean_matrices[[i]] <- mean_matrix
#         } else {
#             mean_matrices[[i]] <- NULL  # Placeholder for no valid data
#         }
#     }
# 
#     return(mean_matrices)
