...
my.batch.pulsar <- function(data, fun=huge::huge, fargs=list(),
    ...
    for (i in 1:length(criterion)) {
        ...
        # if (crit == "gcd") {
        #   
        #     gcdaggfun <- function(res) lapply(res, my.gcvec) # Function to calculate gcv
        #     gcdpremerge <- c(reduceGCDargs$init,
        #                      batchtools::reduceResultsList(reg = reg, ids = pulsar.jobs, fun = gcdaggfun))
        #     
        #     est$gcd <- my.gcd.stability(premerge = gcdpremerge, thresh, rep.num, p, 
        #                                 nlams, prior_graph = prior_graph, use_prior = FALSE, method)
        #  est$gcdpremerge_gcd <- gcdpremerge    
        # }  
        ...
        #                   batchtools::reduceResultsList(reg = reg, ids = pulsar.jobs, fun = gcdaggfun))
        #   
        #   est$gcd_prior <- my.gcd.stability(premerge = gcdpremerge, thresh, rep.num, p, 
        #                                     nlams, prior_graph = prior_graph, use_prior = TRUE, method)
        #   est$gcdpremerge_prior <- gcdpremerge 
        #      
        # } 
        
        if (crit == "gcd" || crit == "gcd_prior") {
            for (meth in method) {
                # Define gcdaggfun for the current method
                gcdaggfun <- function(res, ...) {
                    lapply(res, function(x) my.gcvec(x, method = meth, orbind = orbind, ...))
                }
...
est <- list()
  ...
  if (use_prior == TRUE) {
    if (!method %in% c("kendall", "spearman", "latentcor")) {
      stop("Not supported correlation method is chosen!")
    }
    ...
    for(j in 1:length(premerge)) { # j subsamples
      ...
      for(i in 1:length(premerge[[j]])){ # i lambda values
        ...
        res = dist(rbind(gcv_sub, gcv_true))[[1]] # calculate gcd between (1) gcv of subsample j lambda i and (2) gcv of the true graph
