...
# Ensure that config_labels matches the unique values in mean_f1_df$Config
unique_configs <- unique(mean_f1_df$Config)
config_labels <- sapply(unique_configs, function(cfg_key) {
  ...
  paste(cfg_key, "\nStars:", sparsity_stars, "\nGStars:", sparsity_gstars, "\nOracle:", sparsity_oracle)
}, simplify = FALSE)

# Update the Config column in mean_f1_df and mean_hamming_df
mean_f1_df$Config <- factor(mean_f1_df$Config, levels = unique_configs, labels = config_labels)
mean_hamming_df$Config <- factor(mean_hamming_df$Config, levels = unique_configs, labels = config_labels)
...
config_labels_with_sparsity <- sapply(names(config_labels), function(cfg_key) {
  ...
  paste(config_labels[cfg_key], 
        "<span style='color:lightblue;'>Stars:", sparsity_stars, "</span>",
        "<span style='color:darkorange;'>GStars:", sparsity_gstars, "</span>",
        "<span style='color:black;'>Oracle:", sparsity_oracle, "</span>")
}, simplify = FALSE)
