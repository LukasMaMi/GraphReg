...
## Load Performance Results + Prior
```{r}
...
for (cfg_key in names(config_results)) {
  ...
  for (method in c("Stars", "GStars", "PGStars", "Oracle_f1", "Oracle_hamming", "Null")) {
    # Aggregated metrics
...
## F1 Score and Hamming Distance Plots + Prior
```{r}
...
config_labels_with_sparsity_f1 <- sapply(names(config_labels), function(cfg_key) {
  sparsity_stars <- round(mean_sparsity_df[mean_sparsity_df$Config == cfg_key & 
                                             mean_sparsity_df$Method == "Stars", "Mean_Sparsity"], 3)
  sparsity_gstars <- round(mean_sparsity_df[mean_sparsity_df$Config == cfg_key & 
                                              mean_sparsity_df$Method == "GStars", "Mean_Sparsity"], 3)
  sparsity_pgstars <- round(mean_sparsity_df[mean_sparsity_df$Config == cfg_key & 
                                              mean_sparsity_df$Method == "PGStars", "Mean_Sparsity"], 3)
  sparsity_oracle_f1 <- round(mean_sparsity_df[mean_sparsity_df$Config == cfg_key & 
                                                 mean_sparsity_df$Method == "Oracle_f1", "Mean_Sparsity"], 3)
  ...
  paste(config_labels[cfg_key], 
        ...
        "<span style='color:#009900;'>", sparsity_pgstars, "</span>",
...
config_labels_with_sparsity_hamming <- sapply(names(config_labels), function(cfg_key) {
  ...
  sparsity_pgstars <- round(mean_sparsity_df[mean_sparsity_df$Config == cfg_key & 
                                            mean_sparsity_df$Method == "PGStars", "Mean_Sparsity"], 3)
  sparsity_oracle_hamming <- round(mean_sparsity_df[mean_sparsity_df$Config == cfg_key & 
                                            mean_sparsity_df$Method == "Oracle_hamming", "Mean_Sparsity"], 3)
...
F1_Hub <- ggplot(mean_f1_df_filtered, aes(x = Config, y = Mean_F1, color = Method)) +
  ...
  scale_color_manual(values = c("Stars" = "lightblue", "GStars" = "darkorange", 
                                "PGStars" = "#009900", "Oracle_f1" = "black")) +
...
Hamming_Hub <- ggplot(mean_hamming_df_filtered, aes(x = Config, y = Mean_Hamming, color = Method)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
        ...
        axis.text.x = element_text(angle = 30, hjust = 1)) +
  scale_color_manual(values = c("Stars" = "lightblue", "GStars" = "darkorange", 
                                "PGStars" = "#009900", "Oracle_hamming" = "black", "Null" = "gray")) +
  theme(axis.text.x = ggtext::element_markdown(angle = 30, hjust = 1))
...
for (cfg_key in names(config_results)) {
  ...
  # Loop over methods
  for (method in c("Stars", "GStars", "PGStars", "Oracle_f1", "Oracle_hamming")) {
    aggregated_metrics <- cfg[["Aggregated"]][[tolower(method)]]
...
lambda_plot <- ggplot(mean_lambda_ci_df, aes(x = Config, y = Mean_Lambda, color = Method, group = Method)) +
  ...
  labs(title = "Mean Optim. Lambda Values", 
       y = "Mean Lambda", x = "") +
  theme_minimal() +
  scale_color_manual(values = c("Stars" = "lightblue", "GStars" = "darkorange", "PGStars" = "#009900", 
