...
create_plot <- function(data, gcd_crit, gcd_crit_psd) {
  ...
  # Adding vertical lines for optimal lambda values
    p <- p %>%
...
extract_data <- function(config_results, config, gcd_crit, gcd_crit_psd) {
  ...
  lambda_path <- config_results[[config]][["Lambda_Stats"]][["Mean"]][, 1]
  
  # Extracting Hamming data
  hamming_mean <- config_results[[config]][["Hamming_Stats"]][["Mean"]][, 1]
  hamming_ci <- config_results[[config]][["Hamming_Stats"]][["CI"]]
  
  # Extracting GCD data for the selected criterion
  gcd_mean <- config_results[[config]][["Summary_Stats"]][[gcd_crit]][["Mean"]][, 1]
  gcd_ci <- config_results[[config]][["Summary_Stats"]][[gcd_crit]][["CI"]]
  
  # Extracting GCD data for the selected pseudo criterion
  gcd_mean_psd <- config_results[[config]][["Summary_Stats"]][[gcd_crit_psd]][["Mean"]][, 1]
  gcd_ci_psd <- config_results[[config]][["Summary_Stats"]][[gcd_crit_psd]][["CI"]]
  
  # Extracting optimal lambda values
  lambda_opt_gcd <- config_results[[config]][["Aggregated"]][[gcd_crit]][["Lambda"]][["Mean"]]
  lambda_opt_gcd_psd <- config_results[[config]][["Aggregated"]][[gcd_crit_psd]][["Lambda"]][["Mean"]]
  lambda_opt_hamming <- config_results[[config]][["Aggregated"]][["oracle_hamming"]][["Lambda"]][["Mean"]]
...
extract_criteria <- function(config_results, config) {
  names(config_results[[config]][["Summary_Stats"]])
}
...
# Function to extract data from the config_results
extract_data <- function(config_results, gcd_crit, gcd_crit_psd) {
  lambda_path <- config_results[["n_800_p_40"]][["Lambda_Stats"]][["Mean"]][, 1]
  
  # Extracting Hamming data
  hamming_mean <- config_results[["n_800_p_40"]][["Hamming_Stats"]][["Mean"]][, 1]
  hamming_ci <- config_results[["n_800_p_40"]][["Hamming_Stats"]][["CI"]]
  
  # Extracting GCD data for the selected criterion
  gcd_mean <- config_results[["n_800_p_40"]][["Summary_Stats"]][[gcd_crit]][["Mean"]][, 1]
  gcd_ci <- config_results[["n_800_p_40"]][["Summary_Stats"]][[gcd_crit]][["CI"]]
  
  # Extracting GCD data for the selected criterion
  gcd_mean_psd <- config_results[["n_800_p_40"]][["Summary_Stats"]][[gcd_crit_psd]][["Mean"]][, 1]
  gcd_ci_psd <- config_results[["n_800_p_40"]][["Summary_Stats"]][[gcd_crit_psd]][["CI"]]
  
  # Extracting optimal lambda values
  lambda_opt_gcd <- config_results[["n_800_p_40"]][["Aggregated"]][[gcd_crit]][["Lambda"]][["Mean"]]
  lambda_opt_gcd_psd <- config_results[["n_800_p_40"]][["Aggregated"]][[gcd_crit_psd]][["Lambda"]][["Mean"]]
  lambda_opt_hamming <- config_results[["n_800_p_40"]][["Aggregated"]][["oracle_hamming"]][["Lambda"]][["Mean"]]

  list(lambda_path = lambda_path, hamming_mean = hamming_mean, 
       hamming_ci = hamming_ci, gcd_mean = gcd_mean, gcd_ci = gcd_ci, 
       gcd_mean_psd = gcd_mean_psd, gcd_ci_psd = gcd_ci_psd, 
       lambda_opt_gcd = lambda_opt_gcd, lambda_opt_gcd_psd = lambda_opt_gcd_psd, 
       lambda_opt_hamming = lambda_opt_hamming)
}
...
p <- p %>% layout(
    title = 'Lambda Path Analysis',
    xaxis = list(title = 'Lambda path', title_standoff = 1, showgrid = FALSE),
    yaxis = list(title = 'Hamming Distance', showgrid = FALSE, range = c(0, max_hamming)),
    yaxis2 = list(title = 'GCD', overlaying = 'y',
                  side = 'right',
                  showgrid = FALSE,
    ...
    legend = list(
        x = 0,  # Position the legend to the right of the plot
        y = -0.1,   # Center the legend vertically
        orientation = 'h',  # Vertical orientation of the legend
        font = list(size = 15) 
    ),
    plot_bgcolor = 'rgba(0,0,0,0)',
...
data_extracted <- extract_data(config_results, gcd_crit = "gcd_spearman", gcd_crit_psd = "gcd_pseudo_spearman")
plot <- create_plot(data_extracted, gcd_crit = "gcd_spearman", gcd_crit_psd = "gcd_pseudo_spearman")
...
# Function to extract data from the config_results
extract_data <- function(config_results, gcd_criteria) {
  lambda_path <- config_results[["n_800_p_40"]][["Lambda_Stats"]][["Mean"]][,1]
  ...
  # Extracting GCD data using lapply
  gcd_means <- lapply(gcd_criteria, function(crit) 
    config_results[["n_800_p_40"]][["Summary_Stats"]][[crit]][["Mean"]][,1])
  gcd_cis <- lapply(gcd_criteria, function(crit) 
    config_results[["n_800_p_40"]][["Summary_Stats"]][[crit]][["CI"]])
  
  list(lambda_path = lambda_path, hamming_mean = hamming_mean, 
       hamming_ci = hamming_ci, gcd_means = gcd_means, gcd_cis = gcd_cis)
}

# Function to create the plot
create_plot <- function(data, gcd_criteria) {
  p <- plot_ly() %>%
    add_trace(x = data$lambda_path, y = data$hamming_mean, name = 'Hamming Distance', type = 'scatter', mode = 'lines')

  # Adding ribbons for Hamming CI
  p <- p %>%
    add_ribbons(x = data$lambda_path, y = data$hamming_mean, ymin = data$hamming_ci[,1], ymax = data$hamming_ci[,2],
                name = 'Hamming CI', line = list(color = 'transparent'), fillcolor = 'rgba(255, 127, 80, 0.2)', showlegend = FALSE)

  for(i in 1:length(gcd_criteria)) {
  crit_name <- gcd_criteria[i]
  crit_color <- ifelse(i == 1, 'red', 'green') # Change colors as needed

  # Adding mean GCD lines
  p <- p %>%
    add_trace(x = data$lambda_path, y = data$gcd_means[[i]], name = paste('GCD', crit_name),
  type = 'scatter', mode = 'lines', yaxis = 'y2', line = list(color = crit_color))
  
  # Add dashed lines for GCD CI
  p <- p %>%
    add_trace(x = data$lambda_path, y = data$gcd_cis[[i]][,1], name = paste('GCD', crit_name, 'CI Lower'), 
              type = 'scatter', mode = 'lines', line = list(dash = 'dash', color = crit_color), yaxis = 'y2') %>%
    add_trace(x = data$lambda_path, y = data$gcd_cis[[i]][,2], name = paste('GCD', crit_name, 'CI Upper'), 
              type = 'scatter', mode = 'lines', line = list(dash = 'dash', color = crit_color), yaxis = 'y2')
  ...
  p <- p %>% layout(
title = 'Lambda Path Analysis',
xaxis = list(title = 'Lambda', showgrid = FALSE),
yaxis = list(title = 'Hamming Distance', showgrid = FALSE),
yaxis2 = list(title = 'GCD Values', overlaying = 'y', side = 'right', showgrid = FALSE),
...
gcd_criteria_selected <- c("gcd_pseudo_spearman", "gcd_pseudo_latentcor") # Specify your GCD criteria
data_extracted <- extract_data(config_results, gcd_criteria_selected)
plot <- create_plot(data_extracted, gcd_criteria_selected)
