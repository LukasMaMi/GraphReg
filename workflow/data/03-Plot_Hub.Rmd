...
create_plot <- function(data, gcd_crit, gcd_crit_psd) {
  ...
  # Adding mean Stars line
  p <- p %>%
    add_trace(x = data$lambda_path, y = data$stars_mean, name = "StARS",
              type = 'scatter', mode = 'lines', yaxis = 'y2', line = list(color = 'rgba(173, 216, 230, 1)'))
  
  # Adding ribbons for Stars CI
  p <- p %>%
    add_ribbons(x = data$lambda_path, y = data$stars_mean, ymin = data$stars_ci[, 1], ymax = data$stars_ci[, 2],
                name = paste('StARS CI'), yaxis = 'y2', line = list(color = 'transparent'), 
                fillcolor = 'rgba(173, 216, 230, 0.2)', showlegend = FALSE)
  
  
  
  # Calculate the range for y2-axis (GCD values)
  max_gcd <- max(c(data$gcd_mean, data$gcd_mean_psd, as.vector(data$gcd_ci), as.vector(data$gcd_ci_psd)), na.rm = TRUE)
  max_hamming <- max(c(data$hamming_mean, as.vector(data$hamming_ci)), na.rm = TRUE)
...
extract_data <- function(config_results, config, gcd_crit, gcd_crit_psd) {
  ...
  stars_mean <- config_results[[config]][["Summary_Path"]][["stars"]][["Mean"]]
  stars_ci <- config_results[[config]][["Summary_Path"]][["stars"]][["CI"]]
  ...
  list(lambda_path = lambda_path, hamming_mean = hamming_mean, hamming_ci = hamming_ci, gcd_mean = gcd_mean, gcd_ci = gcd_ci, 
       ...
       stars_mean = stars_mean, stars_ci = stars_ci)
}
