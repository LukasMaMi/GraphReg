...
if(cfg$n == 800 && cfg$p == 40) {
    ...
    for(rep in 1:num_repetitions) {
        ...
        #Store individual summary values
        summary <- list()
        for(method_name in categorized_info[["criterion"]]) {
          if (startsWith(method_name, "gcd_")) {
            summary[[method_name]] <- categorized_info[["raw_summary"]][[method_name]]
            summary_values[[rep]] <- summary
          }
        }
    ...
    # for (method_name in categorized_info[["criterion"]]) {
    #     aggregated_summary_stats <- calc_mean_ci_summary(summary_values[[method_name]])
    # }
...
for(cfg in configs) {
    ...
    if(cfg$n == 800 && cfg$p == 40) {  # Check if the configuration matches n = 800, p = 40
        for(rep in 1:num_repetitions) {
            filename <- get_filename(cfg, rep, "estimation")
            print(paste("Loading data from:", filename))
            load(paste0(dir_path_results, "/", filename))
            
            # Save the loaded data into the list with a unique key
            results_list[[paste(cfg_key, "rep", rep, sep="_")]] <- categorized_info
        }
    }
}
...
for(rep in 1:num_repetitions) {
    # Construct the key dynamically based on the repetition number
    key <- paste("n_800_p_40_rep", rep, sep="_")

    # Access the hamming distance for the current repetition
    hamming_dist_for_rep <- results_list[[key]][["additional_metrics"]][["hamming_dist"]]
    lambda_bound_for_rep <- results_list[[key]][["additional_metrics"]][["lambda_bound"]]
    gcd_spearman_for_rep <- results_list[[key]][["raw_summary"]][["gcd_spearman"]]
    f1_score_for_rep <- results_list[[key]][["additional_metrics"]][["f1_score"]]

    # Store the hamming distance in the hamming_dists list
    hamming_dists[[rep]] <- hamming_dist_for_rep
    lambda_bound[[rep]] <- lambda_bound_for_rep
    gcd_spearman[[rep]] <- gcd_spearman_for_rep 
    f1_score[[rep]] <- f1_score_for_rep
}

# Now you can apply your function to the hamming_dists list
aggregated_hamming_stats <- calc_mean_ci_summary(hamming_dists)
aggregated_lambda_stats <- calc_mean_ci_summary(lambda_bound)
aggregated_f1_stats <- calc_mean_ci_summary(f1_score)
aggregated_gcd_spearman <- calc_mean_ci_summary(gcd_spearman)
