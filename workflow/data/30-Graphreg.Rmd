...
## Version 1
# Keeping NaNs, lb and ub needs to be turned on or small constant added to denominator.
...
# ## Version 2
# # Only remove the subsample which contain NaN, lb and ub can be turned off
# ggd.stability <- function(premerge, rep.num, nlams, prior_graph = NULL, use_prior = FALSE, ...) {
#   est <- list()
# 
#   if (use_prior && !is.null(prior_graph)) {
#     ggv_true <- ggvec(prior_graph, ...)
#     est$merge <- vector("list", nlams)
#     for (i in 1:nlams) {
#       lambda_distances <- numeric(rep.num)
#       for (j in 1:rep.num) {
...
#           lambda_distances[j] <- dist(rbind(ggv_sub, ggv_true))[[1]]
#         } else {
#           valid_indices <- which(is.finite(ggv_true) & is.finite(ggv_sub))
#           if (length(valid_indices) > 0) {
#             lambda_distances[j] <- dist(rbind(ggv_sub[valid_indices], ggv_true[valid_indices]))[[1]]
#           } else {
#             lambda_distances[j] <- NA  # Assign NA if no valid comparison can be made
#           }
#         }
#       }
#       est$merge[[i]] <- lambda_distances
...
## Version 3
# If one NaN remove whole lambda, lb and ub can be turned off
ggd.stability <- function(premerge, rep.num, nlams, prior_graph = NULL, use_prior = FALSE, ...) {
  est <- list()
  ...
  if (use_prior && !is.null(prior_graph)) {
    ...
    if (all(is.finite(ggv_true))) {
      ...
      print("GHuST vector of true graph contains NaN/NA value \n --> Skipping ghust_prior calculation!")
    }
  } else {
