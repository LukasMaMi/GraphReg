...
Graphreg <- function(data, fun = huge::huge, fargs=list(),
    ...
    n <- nrow(data)
    p <- ncol(data)
    knowncrits <- c("stars", "gcd", "gcd_prior", "ghust")
    .lamcheck(fargs$lambda)
    .critcheck0(criterion, knowncrits)
    
    if ("gcd_prior" %in% criterion && is.null(prior_graph)) {
...
# ggd.stability <- function(premerge, rep.num, nlams, ...) {
#   est <- list()
# 
#   est$merge <- lapply(1:nlams, function(i) {
#     # Initialize a matrix to store distances for the lambda
#     distance_matrix <- matrix(nrow = rep.num, ncol = rep.num)
#     for (j in 1:(rep.num - 1)) {
#       for (k in (j + 1):rep.num) {
#         vec1 <- premerge[[j]][[i]]
#         vec2 <- premerge[[k]][[i]]
#         n_missing <- sum(is.na(vec1) | is.na(vec2))
#         distance <- as.numeric(dist(rbind(vec1, vec2)))
#         if (n_missing > 0) {
#           distance_matrix[j, k] <- distance * ((12 + n_missing) / 12)
#         } else {
#           distance_matrix[j, k] <- distance
#         }
#       }
#     }
#     # Return the upper triangle of the matrix as this contains all unique pair-wise comparisons
#     distance_matrix
#   })
# 
#   est$summary <- sapply(est$merge, function(matrix) {
#     mean(matrix[upper.tri(matrix)])
#   })
# 
#   return(est)
# }
...
ggvec <- function(adj_matrix) { #graphlet ghust vector
  ...
  #cat("sum(Pt[, O0]):", sum(Pt[, "O0"]), "\n")
  #cat("sum(gdm[, O0]):", sum(gdm[, "O0"]), "\n")
