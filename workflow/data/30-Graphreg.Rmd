...
Graphreg <- function(data, fun = huge::huge, fargs=list(),
    ...
    if (refit == TRUE) {
      ...
      for (crit in criterion) {
        # Initialize opt_ind with NA or appropriate default if it's not already set
        opt_ind <- if (!is.null(est[[crit]]$opt.index)) est[[crit]]$opt.index else NA
      
        # Now, check if opt_ind is valid before using it
        if (!is.na(opt_ind) && is.numeric(opt_ind) && opt_ind > 0 && opt_ind <= length(fullmodel$path)) {
