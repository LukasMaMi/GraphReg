...
### Integrate this into pulsar at some point
set_opt_indices <- function(obj, indices) {
    for (crit in names(indices)) {
        if (!is.na(indices[[crit]])) {
            obj <- "my.opt.index<-"(obj, criterion = crit, value = indices[[crit]])
        }
    }
    return(obj)
}

# Extract optimal Index
get_opt_indices <- function(obj, criteria) {
    indices <- list()
    for (crit in criteria) {
        opt_index <- my.get.opt.index(obj, criterion = crit)
        indices[[crit]] <- opt_index  # Directly assigning the index
    }
    return(indices)
}


get_optimal_lambda <- function(opt_indices, lambda_path) {
    optimal_lambda <- list()
    for (crit in names(opt_indices)) {
        index <- opt_indices[[crit]]
        if (!is.na(index) && index <= length(lambda_path)) {
            optimal_lambda[[crit]] <- round(lambda_path[index], 3)
        } else {
            optimal_lambda[[crit]] <- NA
        }
    }
...
criteria <- unlist(out.p$criterion)
criteria <- criteria[!criteria %in% c("gcd", "gcd_prior")]
opt_indices <- get_opt_indices(out.p, criteria)
out.p <- set_opt_indices(out.p, opt_indices)
optimal_lambda_values <- get_optimal_lambda(opt_indices, lambda_path = lambda$lambda)
...
result <- list()
for (crit in criteria) {
    result[[crit]] <- list("Optimal Index" = opt_indices[[crit]])
}
...
# Optimal Index Gstars_spearman
my.opt.index(out.p, criterion = "gcd") <- my.get.opt.index(out.p, criterion = "gcd")
gstars_index_s <- my.opt.index(out.p, 'gcd')[[1]]
# Optimal Lambda
best_lambda_gstars_spearman <- round(lambda_path[gstars_index_spearman], 3)

# Optimal Index PG-stars
my.opt.index(out.p, criterion = "gcd_prior_spearman") <- my.get.opt.index(out.p, criterion = "gcd_prior_spearman")
pgstars_index <- my.opt.index(out.p, 'gcd_prior_spearman')[[1]]
# Optimal Lambda
