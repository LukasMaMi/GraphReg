...
my.batch.pulsar <- function(data, fun=huge::huge, fargs=list(),
    ...
    # It's used in reduceResults to combine results from different subsamples.
    ...
    if (lb.stars) {
        ...
        if ('gcd' %in% criterion) { # lb.gcdpremerge aggregates pre-reduction results for GCD, useful for later calculations. !!!
            ...
            # reduceGCDargs provides extra settings or parameters that modify how 
            #batchtools::reduceResultsList processes the job results i.e. reduces it. !!!
        }
    ...
    # Compute full GCD data before any reduction
    # Print statement to check the structure of fullGCDmerge and fullGCDdata
    print("Computing full GCD data...")
    fullGCDmerge <- batchtools::reduceResultsList(reg = reg, ids = pulsar.jobs, fun = function(job, res) lapply(res, gcvec))
    fullGCDdata <- lapply(1:nlams, function(i) dist(t(sapply(1:rep.num, function(j) fullGCDmerge[[j]][[i]]))))
    print("Structure of fullGCDmerge:")
    print(str(fullGCDmerge))
    print("Structure of fullGCDdata:")
    print(str(fullGCDdata))

    # Store full GCD data
    est$fullGCD <- list(
        merge = fullGCDdata,
        summary = sapply(fullGCDdata, mean)
    )
...
my.batch.pulsar <- function(data, fun=huge::huge, fargs=list(),
    ...
    # Insert after handling lb.stars and ub.stars, and before the criterion loop                                        # !!!!
    # Compute full GCD data before any reduction
    fullGCDmerge <- batchtools::reduceResultsList(reg=reg, ids=pulsar.jobs, fun=function(job, res) lapply(res, gcvec))
    fullGCDdata <- lapply(1:nlams, function(i) dist(t(sapply(1:rep.num, function(j) fullGCDmerge[[j]][[i]]))))
    ...
    # Insert just before the return statement of the function                 # !!!!
    # Store full GCD data
    est$fullGCD <- list(
      merge = fullGCDdata,
      summary = sapply(fullGCDdata, mean)
