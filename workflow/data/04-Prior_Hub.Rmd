...
```{r}
    ...
    gcd_criteria <- grep("^gcd_\\w+$|^ghust$", criteria, value = TRUE)
    gcd_prior_criteria <- grep("^gcd_prior_\\w+$|^ghust_prior$", criteria, value = TRUE)
    
          if (startsWith(crit, "gcd")) {
        gcd_prior_pseudo_crit <- paste0("gcd_prior_pseudo_", gsub("^gcd_prior_", "", crit))
      }
      
      else if (startsWith(crit, "ghust")) {
        gcd_prior_pseudo_crit <- paste0("ghust_prior")
      }
