...
modify_orb_count <- function(orb_count, modification_prob = 0.5, delta = 0.1) {
  # Iterate over each element of the matrix
  for (i in 1:nrow(orb_count)) {
    for (j in 1:ncol(orb_count)) {
      # Decide whether to modify this element
      if (runif(1) < modification_prob) {
        # Randomly choose to add or subtract
        if (runif(1) < 0.5) {
          orb_count[i, j] <- orb_count[i, j] + delta
        } else {
          orb_count[i, j] <- orb_count[i, j] - delta
        }
      }
...
gcm_norm <- suppressWarnings(cor(orb_count_normal, method = method))
gcm_norm
gcm_rand <- suppressWarnings(cor(orb_count_random, method = method))
gcm_rand

gcv <- gcm[upper.tri(gcm)] 
gcv
