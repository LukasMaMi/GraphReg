...
generator_geom <- function (n, p, rho, vis, verbose) 
{
    ...
    for (i in 1:(p-1)) {
      for (j in (i+1):p) {
        ...
        # Checks whether current row have fewer non-zero off-diagonal elements than the allowed
        if (sum(omega[i, -i] != 0) < floor(1/rho) && sum(omega[-j, j] != 0) < floor(1/rho)) { 
          
          # Calculates the Euclidean distance between the points in the unit square.
          distance <- sqrt(sum((y[i, ] - y[j, ])^2)) 
          
          # Randomly determines, based on probability, whether to place an edge (non-zero value) between nodes 
