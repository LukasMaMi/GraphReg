...
dir_path_lam <- "path/to/lambda/path/plots"
dir_GCM <- "path/to/GCM/nonpseudo"
dir_GCM_psd <- "path/to/GCM/pseudo"
dir_heatmaps <- "path/to/heatmaps"
dir_subsamples <- "path/to/subsamples"

# Example configuration and criteria
config <- "n_800_p_40"
criteria <- c("gcd_kendall", "gcd_pseudo_kendall", "oracle_hamming", "true_graph")

# Function to read and resize an image
read_and_resize <- function(path, size = "200x200") {
  image <- image_read(path, density = 300)
  image_resized <- image_scale(image, size)
  return(image_resized)
}

# Function to generate file paths based on config and criteria
generate_file_paths <- function(config, criterion, dir_path) {
  file_path <- file.path(dir_path, paste(config, criterion, "pdf", sep = "_"))
  return(file_path)
}
...
for (criterion in criteria) {
  # Lambda path plot (for illustration, assuming it matches one of the criteria)
  if (criterion == "oracle_hamming") {
    lambda_path_plot <- read_and_resize(generate_file_paths(config, criterion, dir_path_lam))
    processed_images[[criterion]] <- lambda_path_plot
  }
  
  # GCM and heatmap plots (adjust as needed for your specific criteria and file organization)
  gcm_plot <- read_and_resize(generate_file_paths(config, criterion, dir_GCM))
  heatmap_plot <- read_and_resize(generate_file_paths(config, criterion, dir_heatmaps))
  processed_images[[paste(criterion, "gcm", sep = "_")]] <- gcm_plot
  processed_images[[paste(criterion, "heatmap", sep = "_")]] <- heatmap_plot
  
  # Subsample plots (assuming a naming convention and directory structure you need to specify)
  for (subsample_index in 1:3) {
    subsample_name <- paste("oracle_hamming_subsample", subsample_index)
    subsample_gcm_plot <- read_and_resize(generate_file_paths(config, subsample_name, dir_subsamples))
    subsample_heatmap_plot <- read_and_resize(generate_file_paths(config, subsample_name, dir_subsamples))
    processed_images[[paste(subsample_name, "gcm", sep = "_")]] <- subsample_gcm_plot
    processed_images[[paste(subsample_name, "heatmap", sep = "_")]] <- subsample_heatmap_plot
  }
}

# Example of compositing (you will need to adjust positions and possibly iterate over your images)
composite_image <- image_composite(processed_images[[1]], processed_images[[2]], offset = "+50+50")
for (i in 3:length(processed_images)) {
  composite_image <- image_composite(composite_image, processed_images[[i]], offset = "+260+50")  # Adjust positioning
}

# Save the composite image
composite_file_path <- file.path(dir_path_lam, paste(config, "composite", "pdf", sep = "_"))
image_write(composite_image, path = composite_file_path, format = "pdf")
...
dir_path_lam <- "/Users/bropc/Documents/LMU/Master Statistics and Data Science/Masterarbeit/R Master/StARS_Simulations/workflow/Storage_Plot_Geom/Lam_path_plots"
dir_GCM_psd <- "/Users/bropc/Documents/LMU/Master Statistics and Data Science/Masterarbeit/R Master/StARS_Simulations/workflow/Storage_Plot_Geom/GCM/pseudo"
dir_GCM <- "/Users/bropc/Documents/LMU/Master Statistics and Data Science/Masterarbeit/R Master/StARS_Simulations/workflow/Storage_Plot_Geom/GCM/nonpseudo"

## Convert PDFs to images
image_lam_path <- image_read(file.path(dir_path_lam, "n_800_p_40_gcd_spearman.pdf"), density = 300)

# GCMs
GCM_gcd <- image_read(file.path(dir_GCM, "n_800_p_40_gcd_spearman.pdf"), density = 300)
GCM_gcd_psd <- image_read(file.path(dir_GCM_psd, "n_800_p_40_true_graph_kendall.pdf"), density = 300)
GCM_oracle <- image_read(file.path(dir_GCM, "n_800_p_40_true_graph_spearman.pdf"), density = 300)

# Heatmaps
Heat_gcd <- image_read(file.path(dir_path6, "n_800_p_40_true_graph_spearman.pdf"), density = 300)
Heat_gcd_psd <- image_read(file.path(dir_path6, "n_800_p_40_true_graph_spearman.pdf"), density = 300)
Heat_oracle <- image_read(file.path(dir_path6, "n_800_p_40_true_graph_spearman.pdf"), density = 300)
...
create_plot <- function(data, gcd_crit, gcd_crit_psd) {
  ...
  p <- p %>% layout(
      ...
      plot_bgcolor = 'rgba(0,0,0,0)',
      margin = list(l = 150, r = 50, t = 150, b = 150)
  )
