...
```{r}
# This file is part of the master's thesis project: "GraphReg: An Extended 
# Stability Approach to Regularization Selection in Sparse Graphical Models".
#
# "GraphReg: An Extended Stability Approach to Regularization Selection in 
# Sparse Graphical Models" is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# "GraphReg: An Extended Stability Approach to Regularization Selection in 
# Sparse Graphical Models" is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with "GraphReg: An Extended Stability Approach to Regularization Selection in 
# Sparse Graphical Models". If not, see <https://www.gnu.org/licenses/>.
```
...
Graphreg <- function(data, fun = huge::huge, fargs=list(),
    ...
    for (crit in criterion) {
      if (startsWith(crit, "gcd") || startsWith(crit, "ghust")) {
        ...
        if (any(!is.na(est[[crit]]$summary))) {
          # Not all values are NA, get the index of the minimum non-NA value
          gcdind <- which.min(est[[crit]]$summary)
        } else {
          # All values are NA, set gcdind to NA
          gcdind <- NA
          }
      ...
      est[[crit]]$opt.lambda <- ifelse(!is.na(est[[crit]]$opt.index), signif(lambda_full[est[[crit]]$opt.index], 3), NA)
    }
...
# Version 2: Standard Ghust but consider only complete GGVs - Ghust lb ub
# ggd.stability <- function(premerge, rep.num, nlams, prior_graph = NULL, use_prior = FALSE, ...) {
#   est <- list()
#   est$merge <- vector("list", nlams)  # Initialize merge for all lambda cases to avoid uninitialization issues
# 
...
#           est$merge[[i]] <- lambda_distances
#         } else {
#           est$merge[[i]] <- rep(NA, rep.num) # Assign NA if any NaN present in subsamples
#         }
#       }
#     } else {
#       est$merge <- lapply(est$merge, function(x) rep(NA, rep.num))  # Safely assign NA to all lambda cases
#       print("GHuST vector of true graph contains NaN/NA value --> Skipping ghust_prior calculation!")
#     }
#   } else {
...
ggvec <- function(adj_matrix) { #graphlet ghust vector
  ...
  ### Version 1: Adding small number to prevent division by zero
  epsilon <- 1e-6
